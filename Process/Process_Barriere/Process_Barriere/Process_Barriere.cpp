...
Process_Barriere* Process_Barriere::instance = nullptr;

Process_Barriere* Process_Barriere::getInstance()
{
	if (instance == nullptr)
		instance = new Process_Barriere();
	return instance;
}
...
void Process_Barriere::onClientReadyRead()
{
	...
	QString str(data);
	qDebug() << "\nMessage reçu du client -> " << client->getName() << " : " << str;
	...
	if (!clients.isEmpty()) {
		for (Clients* client : clients) {
			if (client->getName() == "PCQuentin") {
				qDebug() << "\nEnvoi de la demande de reconnaissance de plaque à : " << client->getName();
		...
		qDebug() << "Le client spécifié n'est pas connecté pour envoyer la demande de reconnaissance.";
	}
}
...
{
	/* sendOpenBarriere() : envoi à l'Arduino2 une demande d'ouverture de barrière. */

	if (!clients.isEmpty()) {
		for (Clients* client : clients) {
			if (client->getName() == "Arduino2") {
				qDebug() << "\nEnvoi de la demande d'ouverture de la barriere à : " << client->getName();
		...
		qDebug() << "Le client spécifié n'est pas connecté pour envoyer la demande d'ouverture.";
	}
